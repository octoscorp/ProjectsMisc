...
class View:
    def __init__(self, controller, stations, tracks):
        ...
        self.canvas = Canvas(map_frame, width=int(1000*SCALE_FACTOR), height=int(750*SCALE_FACTOR), highlightthickness=0, bg=BACKGROUND)
        self.canvas.pack()
    ...
    def create_dialog_area(self, frm):
        ...
        title_1_label = Label(s1_frm, text = "Station 1", bd = 0, bg = BACKGROUND, fg=SELECTED, font = ("Arial", 15))
        title_1_label.pack(side = TOP, anchor = NW)

        self.station_1_label = Label(s1_frm, text = "-", pady = 5, width = 26, bd = 0, bg = BACKGROUND, fg=SELECTED, font = ("Arial", 20))
        ...
        title_2_label = Label(s2_frm, text = "Station 2", bd = 0, bg = BACKGROUND, fg=SELECTED, font = ("Arial", 15))
        title_2_label.pack(side = TOP, anchor = NW)

        self.station_2_label = Label(s2_frm, text = "-", pady = 5, width = 26, bd = 0, bg = BACKGROUND, fg=SELECTED, font = ("Arial", 20))
        self.station_2_label.pack(side = TOP)

        self.all_routes_btn = Button(s2_frm, bg = BUTTON, text = "Get most routes", command = self.all_routes, state=DISABLED)
        self.all_routes_btn.pack(side = LEFT)

        self.shortest_route_btn = Button(s2_frm, bg = BUTTON, text = "Get shortest route", command = self.shortest_route, state=DISABLED)
        ...
        tracks_lbl = Label(r_frm, text = "Route Selected", bd = 0, bg = BACKGROUND, fg=SELECTED, font = ("Arial", 14))
        tracks_lbl.pack(side = TOP, anchor = NW)

        self.tracks_num_lbl = Label(r_frm, text = "-", pady = 5, bd = 0, bg = BACKGROUND, fg=SELECTED, font = ("Arial", 12))
        self.tracks_num_lbl.pack(side = TOP)

        self.tracks_dist_lbl = Label(r_frm, text = "-", pady = 5, bd = 0, bg = BACKGROUND, fg=SELECTED, font = ("Arial", 12))
        self.tracks_dist_lbl.pack(side = TOP)

        self.prev_path_btn = Button(r_frm, bg = BUTTON, text = "Previous", command = self.prev_path, state=DISABLED)
        self.prev_path_btn.pack(side = LEFT)

        self.deselect_all_btn = Button(r_frm, bg = BUTTON, text = "Deselect tracks", command = self.deselect_all, state=DISABLED)
        self.deselect_all_btn.pack(side = LEFT)

        self.next_path_btn = Button(r_frm, bg = BUTTON, text = "Next", command = self.next_path, state=DISABLED)
        ...
        self.msg_lbl = Label(s_frm, text = "-", pady = 5, bd = 0, bg = BACKGROUND, fg=BROKEN, font = ("Arial", 12))
    ...
    def create_stations(self, station_list):
        ...
        for station in station_list:
            station_id = self.canvas.create_oval(int((station.position.x-(S_SIZE/2))*SCALE_FACTOR), int((station.position.y-(S_SIZE/2))*SCALE_FACTOR),
                                                 int((station.position.x+1+(S_SIZE/2))*SCALE_FACTOR), int((station.position.y+1+(S_SIZE/2))*SCALE_FACTOR),
                                                 width=0, fill=STATION)
            self.canvas.tag_bind(station_id, '<Button-1>', partial(self.on_station_l_click, station_id))
            self.canvas.tag_bind(station_id, '<Button-3>', partial(self.on_station_r_click, station_id))
    ...
    def create_tracks(self, track_list):
        ...
        for track in track_list:
            if track[0].id in express_ends and track[1].id in express_ends:
                # This hacky workaround is to make a curve, as this one track makes the map look much worse when it goes straight.
                track_id = self.canvas.create_line(int(track[0].position.x*SCALE_FACTOR), int(track[0].position.y*SCALE_FACTOR),
                                                   int(600*SCALE_FACTOR), int(920*SCALE_FACTOR), int(track[1].position.x*SCALE_FACTOR),
                                                   int(track[1].position.y*SCALE_FACTOR), width=int(T_WIDTH*SCALE_FACTOR), dash=(10,3),
                                                   fill=TRACK, smooth=1)
            else:
                track_id = self.canvas.create_line(int(track[0].position.x*SCALE_FACTOR), int(track[0].position.y*SCALE_FACTOR),
                                                   int(track[1].position.x*SCALE_FACTOR), int(track[1].position.y*SCALE_FACTOR),
                                                   width=int(T_WIDTH*SCALE_FACTOR), dash=(10,3), fill=TRACK)
            self.canvas.tag_bind(track_id, '<Button-1>', partial(self.on_track_l_click, track_id))
            self.canvas.tag_bind(track_id, '<Button-3>', partial(self.on_track_r_click, track_id))
    ...
    def redraw(self):
        # Stations
        lbl_contents = "-" if self.selected_station_1 == None else self.stations[self.station_ids.index(self.selected_station_1)].name
        self.station_1_label.config(text=lbl_contents)

        lbl_contents = "-" if self.selected_station_2 == None else self.stations[self.station_ids.index(self.selected_station_2)].name
        self.station_2_label.config(text=lbl_contents)

        btn_state = DISABLED if self.selected_station_1 == None or self.selected_station_2 == None else NORMAL
        self.all_routes_btn.config(state=btn_state)
        self.shortest_route_btn.config(state=btn_state)

        # Routes
        lbl_contents = "-" if len(self.selected_tracks) == 0 else f"{len(self.selected_tracks)} selected"
        self.tracks_num_lbl.config(text=lbl_contents)

        lbl_contents = "-" if len(self.selected_tracks) == 0 else f"{sum([self.tracks[self.track_ids.index(tk_id)][2] for tk_id in self.selected_tracks])}km"
    ...
    def on_track_r_click(self, track_id, _):
        ...
        if track_id in self.deactivated_tracks:
            t = Thread(target=self.controller.activate_track, args=(self.track_ids.index(track_id), self.track_reactivated))
        else:
            t = Thread(target=self.controller.deactivate_track, args=(self.track_ids.index(track_id), self.track_deactivated))
        t.start()
    
    def all_routes(self):
        t = Thread(target=self.controller.all_paths_between, args=(self.station_ids.index(self.selected_station_1),
                                                                   self.station_ids.index(self.selected_station_2),
                                                                   self.pathset_returned))
        t.start()

    def shortest_route(self):
        t = Thread(target=self.controller.shortest_path_between, args=(self.station_ids.index(self.selected_station_1),
                                                                       self.station_ids.index(self.selected_station_2),
                                                                       self.pathset_returned))
        t.start()
    ...
    def station_prompt(self, timestamp, train, station):
        ...
        message = f'{self.controller.trains[train].name} is going through {self.stations[station].name}. Would you like to stop?'
        Label(new_window, text=message).pack()
        Button(new_window, text="No", command=lambda: self.pass_station(train, station, new_window)).pack()
        Button(new_window, text="Yes", command=lambda: self.stop_at_station(train, station, new_window)).pack()
    ...
    def talk_prompt(self, timestamp, train, item):
        ...
        message = f'{self.controller.trains[train].name} and {self.controller.trains[item].name} have a chance to talk as they pass.'
        Label(new_window, text=message).pack()
...
class TrainWindow:
    ...
    def change_facing(self, *args):
        ...
        if value != '' and self.train_object != None:
            ident = None
    ...
    def rerender(self):
        if self.train_object is not None:
            ...
            new_choices = [self.controller.model.nodes[stat].name for stat in self.controller.get_adjacent_nodes(self.train_object.last_station)]
            for choice in new_choices:
                self.facing_select['menu'].add_command(label=choice, command=tk._setit(self.facing, choice))
...
class PuzzleWindow:
    ...
    def __init__(self, window, controller):
        ...
        Label(self.window, text="-----------------------------------").grid(row=5, column=0, columnspan=3)
    ...
    def generate(self):
        ...
        if strt != '' and ls != '' and rs != '':
            t = Thread(target=self.controller.crane_puzzle_numbers, args=(int(strt), int(ls), int(rs), self.update))
            t.start()
